...
def get_malpedia_stix():
    ...
    references = {o["url"]: o for o in bibtexparser.loads(requests.get(URL_BIBTEX).content, bibtex_parser).entries}
...
def disambiguate_aliases(misp):
    objs_with_aliases = [obj for obj in misp["values"] if "meta" in obj and "synonyms" in obj["meta"]]
    aliases = [name.lower() for obj in objs_with_aliases for name in obj["meta"]["synonyms"]]
    for obj in objs_with_aliases:
        obj["meta"]["synonyms"] = [name for name in obj["meta"]["synonyms"] if aliases.count(name.lower()) == 1]
    return misp
...
def build_bundle(families, misp, references):
    ...
    for key in families:
        ...
        bundle = integrate_new_objs([malware] + intrusion_sets + relationships + reports, bundle)
    malp_report = compile_report(
        URL_MALPEDIA,
        {URL_MALPEDIA: {"date": str(datetime.now()), "title": "Malpedia", "organization": "Fraunhofer FKIE", "language": "englisch"}},
...
def integrate_new_objs(new_objs, bundle):
    ...
    for new_obj in new_objs:
        if new_obj["id"] in ids: bundle.remove([o for o in bundle if o["id"] == new_obj["id"]][0])
...
def build_malware(key, obj, bundle):
    ...
    description = "This Malware object was created based on information from " + malpedia_link + "."
    if obj["updated"]: description += " Last update: " + obj["updated"] + "."
...
def build_intrusion_sets(malware, misp, bundle):
    ...
    for iset in malware["attribution"]:
        existing_objs = [obj for obj in bundle if obj["type"] == "intrusion-set" and obj["name"].lower() == iset.lower()]
        if existing_objs: intrusion_sets.extend(existing_objs)
        else: intrusion_sets.append(compile_intrusion_set(misp, iset))
...
def build_reports(malware, mp_obj, bundle, references):
    ...
    for url in mp_obj["urls"]:
        existing_objs = [stix_obj for stix_obj in bundle + new_reports if stix_obj["type"] == "report" and url in str(stix_obj["external_references"])]
        if existing_objs: new_reports.extend(add_object_ref(existing_objs, malware))
        else: new_reports.append(disambiguate_report_names(compile_report(url, references, [malware]), bundle, new_reports))
    return new_reports
...
def add_object_ref(report_objs, malware):
    ...
    for stix_obj in report_objs:
        enriched_reports.append(stix_obj.new_version(object_refs=list(set(stix_obj["object_refs"] + [malware["id"]]))))
...
def compile_report(url, references, contained_objs):
    ...
    if url in references.keys():
        ...
        if "language" in references[url]: description += "Language: " + references[url]["language"] + "\n"
        if "organization" in references[url]: description += "Organization: " + references[url]["organization"]
        if description.endswith("\n"): description = description[:-1]
    else: date, title = get_alt_meta(url)
...
def get_alt_meta(url):
    ...
    if request and request.status_code < 400 and not url.endswith(".pdf"):
        ...
        title = html.unescape(title_match.group(1).replace("\n", " "))[:500] if title_match and len(title_match.group(1)) > 3 else url
    else:
        date = "1970-01-01T00:00:00Z"
        title_match = re.search(r'.*/(.+?)\.[a-z]{2,4}$', url)
        title = title_match.group(1) if title_match and len(title_match.group(1)) > 3 else url
    return date, title
...
def get_date_from_html(html):
    ...
    for html_element in html_elements: 
        ...
        if time.date() < date.today(): return time.strftime("%Y-%m-%dT%H:%M:%SZ")
...
def find_date_elements(html):
    ...
    html_elements.extend(soup.find_all(class_=re.compile(r'.*(?:meta|published|time|date|header|heading|created|av b aw ax bt|card).*')))
    html_elements.extend(soup.find_all(id=re.compile(r'.*(?:authorposton|footer-info-lastmod|meta).*')))
    html_elements.extend(soup.find_all(item_prop=re.compile(r'.*(?:datePublished|dateCreated).*')))
    html_elements.extend(soup.find_all(datetime_arg=re.compile(r'.+')))
    html_elements.extend(soup.find_all(datetime=re.compile(r'.+')))
    html_elements.extend([t.parent for t in soup.find_all(string=lambda t: t and re.search(r'posted|published|edited|<span class="date">', t) and "\n" not in t)])
    for element in html_elements.copy():
        if element.name == "body": html_elements.remove(element)
        elif element.find_parent(class_=re.compile(r'.*(?:revision|comment|sidebar(?!s)|preview|related|footer|referenc).*'), name=re.compile(r'^(?!body).*')): html_elements.remove(element)
        elif element.find_parent(name=re.compile(r'.*(?:aside|revision|history).*')): html_elements.remove(element)
        elif [w for w in ["related"] if w in element.name]: html_elements.remove(element)
    html_elements.sort(key= lambda x: len(x.text))
    return html_elements


def disambiguate_report_names(new_report, bundle, reports):
    existing_report_names = {stix_obj["name"] for stix_obj in bundle + reports if stix_obj["type"] == "report" and stix_obj["name"].startswith(new_report["name"])}
    if existing_report_names:
        numbers = {int(re.findall(r'\(([0-9]+)\)$', name)[0]) for name in existing_report_names if re.findall(r'\(([0-9]+)\)$', name)}
        if numbers: new_name = new_report["name"] + " (" + str(max(numbers) + 1) + ")"
        else: new_name = new_report["name"] + " (1)"
...
def main():
    ...
    with open("./bundle.json", "w") as f: json.dump(stix, f, indent=4)
