...
class BrokerTest(unittest.TestCase):
        ...
        tests = [
            {
                "misp": {
                    "values": [
                        {"meta": {"synonyms": ["name1", "name2"]}},
                        {"meta": {"synonyms": ["name1", "name3"]}},
                    ]
                },
                "result": {
                    "values": [
            ...
            for obj in test["result"]:
                ...
                self.assertTrue(
                    [
                        o
                        for o in result
                        if {key: o[key] for key in o if key in keys}
                        == {key: obj[key] for key in obj if key in keys}
    ...
    def test_integrate_new_objs(self):
        ...
        for test in tests:
            ...
            for obj in result:
                self.assertIn(obj, test["results"])
            for obj in test["results"]:
        ...
        tests = [
            {
                ...
                "obj": {
                    "updated": "1.1.1970",
                    "description": "<Malware1 description>",
                    "alt_names": ["MW1"],
                    "common_name": "Malware1",
                },
                "result": {
                    "type": "malware",
                    "spec_version": "2.1",
                    ...
                    "name": "Malware1",
                    "description": "<Malware1 description>\nThis Malware object was created based on information from https://malpedia.caad.fkie.fraunhofer.de/details/Malware1. Last update: 1.1.1970.",
                    "is_family": "True",
                    "aliases": "['MW1', 'Malware1']",
                    "revoked": "False",
                    "labels": "['malware']",
                    "confidence": "95",
                },
        ...
        tests = [
            {
                ...
                "result": [
                    {"name": "Attacker2", "type": "intrusion-set"},
                    {"name": "attacker1", "type": "intrusion-set"},
                ],
            }
        ]
    ...
    def test_compile_intrusion_set(self):
        tests = [
            {
                ...
                "result": {
                    ...
                    "spec_version": "2.1",
                    "created_by_ref": MALPEDIA_IDENTITY,
                    "created": "2022-02-04 09:37:25.338005+00:00",
                    "modified": "2022-02-04 09:37:25.338005+00:00",
                    "name": "Name1",
                    "description": "This Intrusion-Set object was created based on information from https://malpedia.caad.fkie.fraunhofer.de and https://raw.githubusercontent.com/MISP/misp-galaxy/main/clusters/threat-actor.json.",
                    "aliases": ["name1a"],
                    "revoked": False,
                    "confidence": 95,
                },
        ...
        for test in tests:
            ...
            result_dict = {
                k: result[k]
                for k in result
                if k != "id" and not isinstance(result[k], stix2.utils.STIXdatetime)
            }
    ...
    def test_build_relationships(self):
        tests = [
            {
                ...
                "intrusion-sets": [
                    {"id": "intrusion-set--11111111-1111-1111-b111-111111111111"}
                ],
                "mp_obj": {"updated": "1.1.1970"},
                "result": [
                    {
                        "source_ref": "intrusion-set--11111111-1111-1111-b111-111111111111",
                        "target_ref": "malware--11111111-1111-1111-a111-111111111111",
        ...
        for test in tests:
            result = build_relationships(
                test["malware"], test["intrusion-sets"], test["mp_obj"]
            )
            for rel in result:
                rel_dict = {
                    key: rel[key]
                    for key in rel
                    if key in {"target_ref", "source_ref", "relationship_type"}
                }
    ...
    def test_build_reports(self):
                ...
                "result": [
                    {
                        ...
                        "published": "1970-01-01T00:00:00Z",
                        "object_refs": [
                            "indicator--c623c031-b7e7-4892-a88d-9d5752096b88",
                            "malware--8f20728a-7e18-4f46-b8e0-0e3d0eebb4d7",
                        ],
                        "labels": ["threat-report"],
                        "confidence": 95,
                        "external_references": [
                            {"source_name": "Example", "url": "http://example.com"}
                        ],
                    }
                ],
            },
                ...
                "result": [
                    {
                        ...
                        "published": "1970-01-01T00:00:00Z",
                        "object_refs": [
                            "indicator--aba91e30-94ff-40ee-af60-4e4aebd6b91e",
                            "malware--8f20728a-7e18-4f46-b8e0-0e3d0eebb4d7",
                        ],
                        "labels": ["threat-report"],
                        "confidence": 95,
        ...
        for test in tests:
            ...
            for obj in test["result"]:
                self.assertTrue(
                    [o for o in result if [o[key] == obj[key] for key in obj]]
                )
    ...
    def test_compile_report(self):
        tests = [
            {
                ...
                "result": {
                    ...
                    "object_refs": ["malware--11111111-1111-1111-a111-111111111111"],
                    "labels": ["threat-report"],
                    "confidence": 95,
                    "external_references": "[ExternalReference(source_name='Name1', url='http://example.com')]",
                },
            }
        ]
        ...
        for test in tests:
            ...
            for key in {"created", "modified", "published"}:
                self.assertTrue(result[key], stix2.utils.STIXdatetime)
            result_dict = {key: str(result[key]) for key in test["result"]}
            for key in test["result"]:
                self.assertEqual(str(test["result"][key]), result_dict[key])

    def test_get_date_from_html(self):
        tests = [{"html": "<date>1.1.1970</date>", "result": "1970-01-01T00:00:00Z"}]
    ...
    def test_find_date_elements(self):
        tests = [
            ...
            {"html": "...<body datetime='1.1.1970'>Word</body>...", "result": []},
            {
            ...
            {"html": "...<related datetime='1.1.1970'>Word</related>...", "result": []},
        ]

        for test in tests:
            result = find_date_elements(test["html"])
            for obj in result:
                self.assertIn(str(obj), test["result"])
            for obj in test["result"]:
                self.assertIn(obj, [str(o) for o in result])
